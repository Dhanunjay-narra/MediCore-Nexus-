"""
MediCore Nexus - Telemedicine Suite (telemedicine) SQLAlchemy 2.0 ORM Models
Tables: telemedicine_video_sessions, webrtc_room_tokens, in_call_clinical_chat_messages, virtual_consultation_transcripts
"""
...
class TelemedicineMasterEntity(Base, TimestampMixin):
    """Primary Master Table for Telemedicine Suite."""
    __tablename__ = "telemedicine_master"
...
class TelemedicineTransactionDetail(Base, TimestampMixin):
    """Transactional Line-Item Child Table for Telemedicine Suite."""
    __tablename__ = "telemedicine_transactions"

    id: Mapped[str] = mapped_column(String(64), primary_key=True, index=True)
    master_id: Mapped[str] = mapped_column(String(64), ForeignKey("telemedicine_master.id"), index=True, nullable=False)
...
class TelemedicineAuditSnapshot(Base, TimestampMixin):
    """Historical Compliance Audit Snapshot for Telemedicine Suite."""
    __tablename__ = "telemedicine_audit_snapshots"
