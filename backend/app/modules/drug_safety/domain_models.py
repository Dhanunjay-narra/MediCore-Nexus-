"""
MediCore Nexus - Clinical Drug Safety (drug_safety) SQLAlchemy 2.0 ORM Models
Tables: drug_drug_interaction_pairings, drug_allergy_cross_reactions, clinical_contraindication_rules, pregnancy_and_lactation_risks
"""
...
class DrugSafetyMasterEntity(Base, TimestampMixin):
    """Primary Master Table for Clinical Drug Safety."""
    __tablename__ = "drug_safety_master"
...
class DrugSafetyTransactionDetail(Base, TimestampMixin):
    """Transactional Line-Item Child Table for Clinical Drug Safety."""
    __tablename__ = "drug_safety_transactions"

    id: Mapped[str] = mapped_column(String(64), primary_key=True, index=True)
    master_id: Mapped[str] = mapped_column(String(64), ForeignKey("drug_safety_master.id"), index=True, nullable=False)
...
class DrugSafetyAuditSnapshot(Base, TimestampMixin):
    """Historical Compliance Audit Snapshot for Clinical Drug Safety."""
    __tablename__ = "drug_safety_audit_snapshots"
