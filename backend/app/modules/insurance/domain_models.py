"""
MediCore Nexus - Insurance & Claims (insurance) SQLAlchemy 2.0 ORM Models
Tables: insurance_payer_entities, patient_insurance_policies, insurance_eligibility_verifications, electronic_claim_submissions, claim_adjudication_items
"""
...
class InsuranceMasterEntity(Base, TimestampMixin):
    """Primary Master Table for Insurance & Claims."""
...
class InsuranceTransactionDetail(Base, TimestampMixin):
    """Transactional Line-Item Child Table for Insurance & Claims."""
    __tablename__ = "insurance_transactions"

    id: Mapped[str] = mapped_column(String(64), primary_key=True, index=True)
    master_id: Mapped[str] = mapped_column(String(64), ForeignKey("insurance_master.id"), index=True, nullable=False)
...
class InsuranceAuditSnapshot(Base, TimestampMixin):
    """Historical Compliance Audit Snapshot for Insurance & Claims."""
    __tablename__ = "insurance_audit_snapshots"
