"""
MediCore Nexus - Insurance & Claims Adjudication (insurance) Pydantic v2 Domain Schemas
...
class InsuranceClaimBaseSchema(BaseModel):
    """Core attributes for InsuranceClaim."""
...
class InsuranceClaimCreateSchema(InsuranceClaimBaseSchema):
    """Payload required to create a new InsuranceClaim record."""
    is_urgent: Optional[bool] = False
    priority_level: Optional[str] = "Normal"


class InsuranceClaimUpdateSchema(BaseModel):
    """Payload permitted for partial update of InsuranceClaim."""
...
class InsuranceClaimFilterSchema(BaseModel):
    """Query parameters for filtering InsuranceClaim collections."""
    status: Optional[str] = None
...
class InsuranceClaimDetailResponseSchema(InsuranceClaimBaseSchema):
    """Detailed response schema for InsuranceClaim with audit timestamps."""
    id: str
    created_at: str
    updated_at: str
...
class InsuranceClaimListResponseSchema(BaseModel):
    """Paginated collection response for InsuranceClaim."""
    items: List[InsuranceClaimDetailResponseSchema]
    total: int
    skip: int
    limit: int
    has_more: bool
...
class InsuranceClaimBulkActionRequest(BaseModel):
    """Batch operation request for InsuranceClaim entities."""
    action: str = Field(..., description="Action to perform: ACTIVATE, ARCHIVE, DELETE, EXPORT")
    entity_ids: List[str] = Field(..., min_length=1)
    reason: Optional[str] = None


class InsuranceClaimBulkActionResponse(BaseModel):
