"""
MediCore Nexus - Hospital & Organization Management (organization) Pydantic v2 Domain Schemas
...
class HospitalBaseSchema(BaseModel):
    """Core attributes for Hospital."""
...
class HospitalCreateSchema(HospitalBaseSchema):
    """Payload required to create a new Hospital record."""
    is_urgent: Optional[bool] = False
    priority_level: Optional[str] = "Normal"


class HospitalUpdateSchema(BaseModel):
    """Payload permitted for partial update of Hospital."""
...
class HospitalFilterSchema(BaseModel):
    """Query parameters for filtering Hospital collections."""
    status: Optional[str] = None
...
class HospitalDetailResponseSchema(HospitalBaseSchema):
    """Detailed response schema for Hospital with audit timestamps."""
    id: str
    created_at: str
    updated_at: str
...
class HospitalListResponseSchema(BaseModel):
    """Paginated collection response for Hospital."""
    items: List[HospitalDetailResponseSchema]
    total: int
    skip: int
    limit: int
    has_more: bool
...
class HospitalBulkActionRequest(BaseModel):
    """Batch operation request for Hospital entities."""
    action: str = Field(..., description="Action to perform: ACTIVATE, ARCHIVE, DELETE, EXPORT")
    entity_ids: List[str] = Field(..., min_length=1)
    reason: Optional[str] = None


class HospitalBulkActionResponse(BaseModel):
