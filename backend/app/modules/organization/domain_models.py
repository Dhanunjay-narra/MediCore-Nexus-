"""
MediCore Nexus - Hospital & Hierarchy (organization) SQLAlchemy 2.0 ORM Models
Tables: organizations, hospital_facilities, clinical_departments, inpatient_wards, hospital_rooms, hospital_beds, clinic_consultation_units
"""
...
class OrganizationMasterEntity(Base, TimestampMixin):
    """Primary Master Table for Hospital & Hierarchy."""
...
class OrganizationTransactionDetail(Base, TimestampMixin):
    """Transactional Line-Item Child Table for Hospital & Hierarchy."""
    __tablename__ = "organization_transactions"

    id: Mapped[str] = mapped_column(String(64), primary_key=True, index=True)
    master_id: Mapped[str] = mapped_column(String(64), ForeignKey("organization_master.id"), index=True, nullable=False)
...
class OrganizationAuditSnapshot(Base, TimestampMixin):
    """Historical Compliance Audit Snapshot for Hospital & Hierarchy."""
    __tablename__ = "organization_audit_snapshots"
