"""
MediCore Nexus - Identity & Access Management (identity) Pydantic v2 Domain Schemas
...
class UserUpdateSchema(BaseModel):
    """Payload permitted for partial update of User."""
...
class UserFilterSchema(BaseModel):
    """Query parameters for filtering User collections."""
    status: Optional[str] = None
...
class UserDetailResponseSchema(UserBaseSchema):
    """Detailed response schema for User with audit timestamps."""
    id: str
    created_at: str
    updated_at: str
...
class UserListResponseSchema(BaseModel):
    """Paginated collection response for User."""
    items: List[UserDetailResponseSchema]
    total: int
    skip: int
    limit: int
    has_more: bool
...
class UserBulkActionRequest(BaseModel):
    """Batch operation request for User entities."""
