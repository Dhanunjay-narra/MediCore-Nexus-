"""
MediCore Nexus - Identity & Security (identity) SQLAlchemy 2.0 ORM Models
Tables: users, user_sessions, role_permissions, mfa_credentials, audit_security_tokens
"""
...
class IdentityMasterEntity(Base, TimestampMixin):
    """Primary Master Table for Identity & Security."""
    __tablename__ = "identity_master"
...
class IdentityTransactionDetail(Base, TimestampMixin):
    """Transactional Line-Item Child Table for Identity & Security."""
    __tablename__ = "identity_transactions"

    id: Mapped[str] = mapped_column(String(64), primary_key=True, index=True)
    master_id: Mapped[str] = mapped_column(String(64), ForeignKey("identity_master.id"), index=True, nullable=False)
...
class IdentityAuditSnapshot(Base, TimestampMixin):
    """Historical Compliance Audit Snapshot for Identity & Security."""
    __tablename__ = "identity_audit_snapshots"
