"""
MediCore Nexus - Identity & Access Management (identity) Async Database Repository
...
logger = logging.getLogger("medicore.repo.identity")


class UserRepository:
    """
    Asynchronous Repository Pattern for User domain entity.
    Provides CRUD operations, complex querying, full-text search, and relational consistency.
    """

    def __init__(self):
        self._table_name = "identity_records"
        ...
        logger.info(f"Initialized UserRepository with zero-latency in-memory cache.")

    async def get_by_id(self, entity_id: str) -> Optional[Dict[str, Any]]:
        """Retrieve single User record by its primary identifier."""
    ...
    async def create(self, record_data: Dict[str, Any], creator_id: str = "SYSTEM") -> Dict[str, Any]:
        """
        Persist a new User record with audit timestamps and UUID primary key.
        """
        entity_id = record_data.get("id") or f"usr-{uuid.uuid4().hex[:8]}"
        ...
        logger.info(f"Persisted new User [{entity_id}] in identity repository.")
        return record

    async def update(self, entity_id: str, updates: Dict[str, Any], updater_id: str = "SYSTEM") -> Optional[Dict[str, Any]]:
        """
        Perform transactional atomic update on User entity.
        """
        if entity_id not in self._data_store:
            logger.warning(f"User with ID '{entity_id}' not found for update.")
            return None

        record = self._data_store[entity_id]
        if record.get("is_deleted"):
            logger.warning(f"Cannot update soft-deleted User '{entity_id}'.")
            return None
...
# Instantiate repository singleton for identity
