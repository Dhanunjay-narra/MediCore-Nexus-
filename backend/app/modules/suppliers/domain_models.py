"""
MediCore Nexus - Supplier Management (suppliers) SQLAlchemy 2.0 ORM Models
Tables: pharmaceutical_supplier_entities, commercial_vendor_contracts, supplier_product_catalog_prices, vendor_performance_reviews
"""
...
class SuppliersMasterEntity(Base, TimestampMixin):
    """Primary Master Table for Supplier Management."""
    __tablename__ = "suppliers_master"
...
class SuppliersTransactionDetail(Base, TimestampMixin):
    """Transactional Line-Item Child Table for Supplier Management."""
    __tablename__ = "suppliers_transactions"

    id: Mapped[str] = mapped_column(String(64), primary_key=True, index=True)
    master_id: Mapped[str] = mapped_column(String(64), ForeignKey("suppliers_master.id"), index=True, nullable=False)
...
class SuppliersAuditSnapshot(Base, TimestampMixin):
    """Historical Compliance Audit Snapshot for Supplier Management."""
    __tablename__ = "suppliers_audit_snapshots"
