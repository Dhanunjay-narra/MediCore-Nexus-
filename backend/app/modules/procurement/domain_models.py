"""
MediCore Nexus - Procurement & POs (procurement) SQLAlchemy 2.0 ORM Models
Tables: purchase_requisitions, purchase_order_headers, purchase_order_line_items, goods_receipt_notes, vendor_invoice_matching_records
"""
...
class ProcurementMasterEntity(Base, TimestampMixin):
    """Primary Master Table for Procurement & POs."""
    __tablename__ = "procurement_master"
...
class ProcurementTransactionDetail(Base, TimestampMixin):
    """Transactional Line-Item Child Table for Procurement & POs."""
    __tablename__ = "procurement_transactions"

    id: Mapped[str] = mapped_column(String(64), primary_key=True, index=True)
    master_id: Mapped[str] = mapped_column(String(64), ForeignKey("procurement_master.id"), index=True, nullable=False)
...
class ProcurementAuditSnapshot(Base, TimestampMixin):
    """Historical Compliance Audit Snapshot for Procurement & POs."""
    __tablename__ = "procurement_audit_snapshots"
