"""
MediCore Nexus - Doctor Management (doctors) SQLAlchemy 2.0 ORM Models
Tables: physician_profiles, medical_specialties, physician_licenses, consultation_fee_tiers, physician_working_schedules, doctor_reviews
"""
...
class DoctorsMasterEntity(Base, TimestampMixin):
    """Primary Master Table for Doctor Management."""
    __tablename__ = "doctors_master"
...
class DoctorsTransactionDetail(Base, TimestampMixin):
    """Transactional Line-Item Child Table for Doctor Management."""
    __tablename__ = "doctors_transactions"

    id: Mapped[str] = mapped_column(String(64), primary_key=True, index=True)
    master_id: Mapped[str] = mapped_column(String(64), ForeignKey("doctors_master.id"), index=True, nullable=False)
...
class DoctorsAuditSnapshot(Base, TimestampMixin):
    """Historical Compliance Audit Snapshot for Doctor Management."""
    __tablename__ = "doctors_audit_snapshots"
