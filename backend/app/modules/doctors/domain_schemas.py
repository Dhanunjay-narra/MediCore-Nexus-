"""
MediCore Nexus - Doctor & Specialist Operations (doctors) Pydantic v2 Domain Schemas
...
class DoctorCreateSchema(DoctorBaseSchema):
    """Payload required to create a new Doctor record."""
    is_urgent: Optional[bool] = False
    priority_level: Optional[str] = "Normal"


class DoctorUpdateSchema(BaseModel):
    """Payload permitted for partial update of Doctor."""
...
class DoctorFilterSchema(BaseModel):
    """Query parameters for filtering Doctor collections."""
    status: Optional[str] = None
...
class DoctorDetailResponseSchema(DoctorBaseSchema):
    """Detailed response schema for Doctor with audit timestamps."""
    id: str
    created_at: str
    updated_at: str
...
class DoctorListResponseSchema(BaseModel):
    """Paginated collection response for Doctor."""
    items: List[DoctorDetailResponseSchema]
    total: int
    skip: int
    limit: int
    has_more: bool
...
class DoctorBulkActionRequest(BaseModel):
    """Batch operation request for Doctor entities."""
    action: str = Field(..., description="Action to perform: ACTIVATE, ARCHIVE, DELETE, EXPORT")
    entity_ids: List[str] = Field(..., min_length=1)
    reason: Optional[str] = None


class DoctorBulkActionResponse(BaseModel):
