"""
MediCore Nexus - Point-of-Sale Engine (sales) SQLAlchemy 2.0 ORM Models
Tables: pos_sales_transactions, pos_cart_line_items, payment_transaction_ledger, electronic_sales_receipts, return_and_refund_records
"""
...
class SalesMasterEntity(Base, TimestampMixin):
    """Primary Master Table for Point-of-Sale Engine."""
...
class SalesTransactionDetail(Base, TimestampMixin):
    """Transactional Line-Item Child Table for Point-of-Sale Engine."""
    __tablename__ = "sales_transactions"

    id: Mapped[str] = mapped_column(String(64), primary_key=True, index=True)
    master_id: Mapped[str] = mapped_column(String(64), ForeignKey("sales_master.id"), index=True, nullable=False)
...
class SalesAuditSnapshot(Base, TimestampMixin):
    """Historical Compliance Audit Snapshot for Point-of-Sale Engine."""
    __tablename__ = "sales_audit_snapshots"
