"""
MediCore Nexus - Inventory & Stock Control (inventory) SQLAlchemy 2.0 ORM Models
Tables: pharmaceutical_inventory_batches, warehouse_facilities, storage_aisles_and_shelves, stock_adjustment_entries, lot_tracking_records
"""
...
class InventoryMasterEntity(Base, TimestampMixin):
    """Primary Master Table for Inventory & Stock Control."""
...
class InventoryTransactionDetail(Base, TimestampMixin):
    """Transactional Line-Item Child Table for Inventory & Stock Control."""
    __tablename__ = "inventory_transactions"

    id: Mapped[str] = mapped_column(String(64), primary_key=True, index=True)
    master_id: Mapped[str] = mapped_column(String(64), ForeignKey("inventory_master.id"), index=True, nullable=False)
...
class InventoryAuditSnapshot(Base, TimestampMixin):
    """Historical Compliance Audit Snapshot for Inventory & Stock Control."""
    __tablename__ = "inventory_audit_snapshots"
