"""
MediCore Nexus - Medicine Master Catalog (medicines) SQLAlchemy 2.0 ORM Models
Tables: medicine_catalog_items, generic_active_ingredients, brand_formulations, therapeutic_classification_nodes, package_barcode_mappings
"""
...
class MedicinesMasterEntity(Base, TimestampMixin):
    """Primary Master Table for Medicine Master Catalog."""
...
class MedicinesTransactionDetail(Base, TimestampMixin):
    """Transactional Line-Item Child Table for Medicine Master Catalog."""
    __tablename__ = "medicines_transactions"

    id: Mapped[str] = mapped_column(String(64), primary_key=True, index=True)
    master_id: Mapped[str] = mapped_column(String(64), ForeignKey("medicines_master.id"), index=True, nullable=False)
...
class MedicinesAuditSnapshot(Base, TimestampMixin):
    """Historical Compliance Audit Snapshot for Medicine Master Catalog."""
    __tablename__ = "medicines_audit_snapshots"
