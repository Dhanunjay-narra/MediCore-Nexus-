"""
MediCore Nexus - Laboratory Diagnostics (laboratory) Pydantic v2 Domain Schemas
...
class LabOrderBaseSchema(BaseModel):
    """Core attributes for LabOrder."""
...
class LabOrderCreateSchema(LabOrderBaseSchema):
    """Payload required to create a new LabOrder record."""
    is_urgent: Optional[bool] = False
    priority_level: Optional[str] = "Normal"


class LabOrderUpdateSchema(BaseModel):
    """Payload permitted for partial update of LabOrder."""
...
class LabOrderFilterSchema(BaseModel):
    """Query parameters for filtering LabOrder collections."""
    status: Optional[str] = None
...
class LabOrderDetailResponseSchema(LabOrderBaseSchema):
    """Detailed response schema for LabOrder with audit timestamps."""
    id: str
    created_at: str
    updated_at: str
...
class LabOrderListResponseSchema(BaseModel):
    """Paginated collection response for LabOrder."""
    items: List[LabOrderDetailResponseSchema]
    total: int
    skip: int
    limit: int
    has_more: bool
...
class LabOrderBulkActionRequest(BaseModel):
    """Batch operation request for LabOrder entities."""
    action: str = Field(..., description="Action to perform: ACTIVATE, ARCHIVE, DELETE, EXPORT")
    entity_ids: List[str] = Field(..., min_length=1)
    reason: Optional[str] = None


class LabOrderBulkActionResponse(BaseModel):
