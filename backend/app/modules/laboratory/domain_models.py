"""
MediCore Nexus - Laboratory Diagnostics (laboratory) SQLAlchemy 2.0 ORM Models
Tables: laboratory_test_packages, patient_specimen_samples, test_result_measurements, loinc_standard_reference_ranges, critical_alert_dispatches
"""
...
class LaboratoryMasterEntity(Base, TimestampMixin):
    """Primary Master Table for Laboratory Diagnostics."""
    __tablename__ = "laboratory_master"
...
class LaboratoryTransactionDetail(Base, TimestampMixin):
    """Transactional Line-Item Child Table for Laboratory Diagnostics."""
    __tablename__ = "laboratory_transactions"

    id: Mapped[str] = mapped_column(String(64), primary_key=True, index=True)
    master_id: Mapped[str] = mapped_column(String(64), ForeignKey("laboratory_master.id"), index=True, nullable=False)
...
class LaboratoryAuditSnapshot(Base, TimestampMixin):
    """Historical Compliance Audit Snapshot for Laboratory Diagnostics."""
    __tablename__ = "laboratory_audit_snapshots"
