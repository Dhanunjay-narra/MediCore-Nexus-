"""
MediCore Nexus - Appointments & Queues (appointments) SQLAlchemy 2.0 ORM Models
Tables: appointment_bookings, waiting_room_queues, triage_tokens, doctor_availability_slots, no_show_incidents
"""
...
class AppointmentsMasterEntity(Base, TimestampMixin):
    """Primary Master Table for Appointments & Queues."""
    __tablename__ = "appointments_master"
...
class AppointmentsTransactionDetail(Base, TimestampMixin):
    """Transactional Line-Item Child Table for Appointments & Queues."""
    __tablename__ = "appointments_transactions"

    id: Mapped[str] = mapped_column(String(64), primary_key=True, index=True)
    master_id: Mapped[str] = mapped_column(String(64), ForeignKey("appointments_master.id"), index=True, nullable=False)
...
class AppointmentsAuditSnapshot(Base, TimestampMixin):
    """Historical Compliance Audit Snapshot for Appointments & Queues."""
    __tablename__ = "appointments_audit_snapshots"
