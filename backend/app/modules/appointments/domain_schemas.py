"""
MediCore Nexus - Appointment & Queue Management (appointments) Pydantic v2 Domain Schemas
...
class AppointmentBaseSchema(BaseModel):
    """Core attributes for Appointment."""
...
class AppointmentCreateSchema(AppointmentBaseSchema):
    """Payload required to create a new Appointment record."""
    is_urgent: Optional[bool] = False
    priority_level: Optional[str] = "Normal"


class AppointmentUpdateSchema(BaseModel):
    """Payload permitted for partial update of Appointment."""
...
class AppointmentFilterSchema(BaseModel):
    """Query parameters for filtering Appointment collections."""
    status: Optional[str] = None
...
class AppointmentDetailResponseSchema(AppointmentBaseSchema):
    """Detailed response schema for Appointment with audit timestamps."""
    id: str
    created_at: str
    updated_at: str
...
class AppointmentListResponseSchema(BaseModel):
    """Paginated collection response for Appointment."""
    items: List[AppointmentDetailResponseSchema]
    total: int
    skip: int
    limit: int
    has_more: bool
...
class AppointmentBulkActionRequest(BaseModel):
    """Batch operation request for Appointment entities."""
    action: str = Field(..., description="Action to perform: ACTIVATE, ARCHIVE, DELETE, EXPORT")
    entity_ids: List[str] = Field(..., min_length=1)
    reason: Optional[str] = None


class AppointmentBulkActionResponse(BaseModel):
