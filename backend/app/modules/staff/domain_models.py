"""
MediCore Nexus - Staff & Rostering (staff) SQLAlchemy 2.0 ORM Models
Tables: hospital_staff_employees, duty_shift_schedules, employee_attendance_punches, counter_station_assignments, performance_scorecards
"""
...
class StaffMasterEntity(Base, TimestampMixin):
    """Primary Master Table for Staff & Rostering."""
...
class StaffTransactionDetail(Base, TimestampMixin):
    """Transactional Line-Item Child Table for Staff & Rostering."""
    __tablename__ = "staff_transactions"

    id: Mapped[str] = mapped_column(String(64), primary_key=True, index=True)
    master_id: Mapped[str] = mapped_column(String(64), ForeignKey("staff_master.id"), index=True, nullable=False)
...
class StaffAuditSnapshot(Base, TimestampMixin):
    """Historical Compliance Audit Snapshot for Staff & Rostering."""
    __tablename__ = "staff_audit_snapshots"
