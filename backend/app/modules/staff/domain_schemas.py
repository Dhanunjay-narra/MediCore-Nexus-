"""
MediCore Nexus - Staff & Roster Management (staff) Pydantic v2 Domain Schemas
...
class StaffMemberBaseSchema(BaseModel):
    """Core attributes for StaffMember."""
...
class StaffMemberCreateSchema(StaffMemberBaseSchema):
    """Payload required to create a new StaffMember record."""
    is_urgent: Optional[bool] = False
    priority_level: Optional[str] = "Normal"


class StaffMemberUpdateSchema(BaseModel):
    """Payload permitted for partial update of StaffMember."""
...
class StaffMemberFilterSchema(BaseModel):
    """Query parameters for filtering StaffMember collections."""
    status: Optional[str] = None
...
class StaffMemberDetailResponseSchema(StaffMemberBaseSchema):
    """Detailed response schema for StaffMember with audit timestamps."""
    id: str
    created_at: str
    updated_at: str
...
class StaffMemberListResponseSchema(BaseModel):
    """Paginated collection response for StaffMember."""
    items: List[StaffMemberDetailResponseSchema]
    total: int
    skip: int
    limit: int
    has_more: bool
...
class StaffMemberBulkActionRequest(BaseModel):
    """Batch operation request for StaffMember entities."""
    action: str = Field(..., description="Action to perform: ACTIVATE, ARCHIVE, DELETE, EXPORT")
    entity_ids: List[str] = Field(..., min_length=1)
    reason: Optional[str] = None


class StaffMemberBulkActionResponse(BaseModel):
