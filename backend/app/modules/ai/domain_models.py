"""
MediCore Nexus - AI Decision Support (ai) SQLAlchemy 2.0 ORM Models
Tables: ai_prescription_anomaly_logs, predictive_stockout_estimations, natural_language_query_histories, clinical_decision_support_insights
"""
...
class AiMasterEntity(Base, TimestampMixin):
    """Primary Master Table for AI Decision Support."""
...
class AiTransactionDetail(Base, TimestampMixin):
    """Transactional Line-Item Child Table for AI Decision Support."""
    __tablename__ = "ai_transactions"

    id: Mapped[str] = mapped_column(String(64), primary_key=True, index=True)
    master_id: Mapped[str] = mapped_column(String(64), ForeignKey("ai_master.id"), index=True, nullable=False)
...
class AiAuditSnapshot(Base, TimestampMixin):
    """Historical Compliance Audit Snapshot for AI Decision Support."""
    __tablename__ = "ai_audit_snapshots"
