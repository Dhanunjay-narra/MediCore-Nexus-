"""
MediCore Nexus - Communications (notifications) SQLAlchemy 2.0 ORM Models
Tables: multichannel_notification_dispatches, sms_gateway_logs, email_delivery_receipts, whatsapp_message_payloads, push_token_registrations
"""
...
class NotificationsMasterEntity(Base, TimestampMixin):
    """Primary Master Table for Communications."""
    __tablename__ = "notifications_master"
...
class NotificationsTransactionDetail(Base, TimestampMixin):
    """Transactional Line-Item Child Table for Communications."""
    __tablename__ = "notifications_transactions"

    id: Mapped[str] = mapped_column(String(64), primary_key=True, index=True)
    master_id: Mapped[str] = mapped_column(String(64), ForeignKey("notifications_master.id"), index=True, nullable=False)
...
class NotificationsAuditSnapshot(Base, TimestampMixin):
    """Historical Compliance Audit Snapshot for Communications."""
    __tablename__ = "notifications_audit_snapshots"
