"""
MediCore Nexus - Document Vault (documents) SQLAlchemy 2.0 ORM Models
Tables: encrypted_clinical_documents, patient_lab_report_attachments, medical_imaging_scans, digital_consent_document_archives
"""
...
class DocumentsMasterEntity(Base, TimestampMixin):
    """Primary Master Table for Document Vault."""
...
class DocumentsTransactionDetail(Base, TimestampMixin):
    """Transactional Line-Item Child Table for Document Vault."""
    __tablename__ = "documents_transactions"

    id: Mapped[str] = mapped_column(String(64), primary_key=True, index=True)
    master_id: Mapped[str] = mapped_column(String(64), ForeignKey("documents_master.id"), index=True, nullable=False)
...
class DocumentsAuditSnapshot(Base, TimestampMixin):
    """Historical Compliance Audit Snapshot for Document Vault."""
    __tablename__ = "documents_audit_snapshots"
