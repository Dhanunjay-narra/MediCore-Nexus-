"""
MediCore Nexus - Pharmacy Operations (pharmacy) SQLAlchemy 2.0 ORM Models
Tables: dispensing_transaction_records, pharmacist_verification_actions, prescription_fulfillment_items, counter_queue_allocations
"""
...
class PharmacyMasterEntity(Base, TimestampMixin):
    """Primary Master Table for Pharmacy Operations."""
    __tablename__ = "pharmacy_master"
...
class PharmacyTransactionDetail(Base, TimestampMixin):
    """Transactional Line-Item Child Table for Pharmacy Operations."""
    __tablename__ = "pharmacy_transactions"

    id: Mapped[str] = mapped_column(String(64), primary_key=True, index=True)
    master_id: Mapped[str] = mapped_column(String(64), ForeignKey("pharmacy_master.id"), index=True, nullable=False)
...
class PharmacyAuditSnapshot(Base, TimestampMixin):
    """Historical Compliance Audit Snapshot for Pharmacy Operations."""
    __tablename__ = "pharmacy_audit_snapshots"
