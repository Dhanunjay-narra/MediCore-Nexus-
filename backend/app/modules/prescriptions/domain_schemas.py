"""
MediCore Nexus - Prescription & E-Prescribing (prescriptions) Pydantic v2 Domain Schemas
...
class PrescriptionBaseSchema(BaseModel):
    """Core attributes for Prescription."""
...
class PrescriptionCreateSchema(PrescriptionBaseSchema):
    """Payload required to create a new Prescription record."""
    is_urgent: Optional[bool] = False
    priority_level: Optional[str] = "Normal"


class PrescriptionUpdateSchema(BaseModel):
    """Payload permitted for partial update of Prescription."""
...
class PrescriptionFilterSchema(BaseModel):
    """Query parameters for filtering Prescription collections."""
    status: Optional[str] = None
...
class PrescriptionDetailResponseSchema(PrescriptionBaseSchema):
    """Detailed response schema for Prescription with audit timestamps."""
    id: str
    created_at: str
    updated_at: str
...
class PrescriptionListResponseSchema(BaseModel):
    """Paginated collection response for Prescription."""
    items: List[PrescriptionDetailResponseSchema]
    total: int
    skip: int
    limit: int
    has_more: bool
...
class PrescriptionBulkActionRequest(BaseModel):
    """Batch operation request for Prescription entities."""
    action: str = Field(..., description="Action to perform: ACTIVATE, ARCHIVE, DELETE, EXPORT")
    entity_ids: List[str] = Field(..., min_length=1)
    reason: Optional[str] = None


class PrescriptionBulkActionResponse(BaseModel):
