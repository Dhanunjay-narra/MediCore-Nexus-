"""
MediCore Nexus - Prescription Operations (prescriptions) SQLAlchemy 2.0 ORM Models
Tables: electronic_prescriptions, prescription_medication_items, digital_signature_audit_hashes, prescription_refill_authorizations
"""
...
class PrescriptionsMasterEntity(Base, TimestampMixin):
    """Primary Master Table for Prescription Operations."""
    __tablename__ = "prescriptions_master"
...
class PrescriptionsTransactionDetail(Base, TimestampMixin):
    """Transactional Line-Item Child Table for Prescription Operations."""
    __tablename__ = "prescriptions_transactions"

    id: Mapped[str] = mapped_column(String(64), primary_key=True, index=True)
    master_id: Mapped[str] = mapped_column(String(64), ForeignKey("prescriptions_master.id"), index=True, nullable=False)
...
class PrescriptionsAuditSnapshot(Base, TimestampMixin):
    """Historical Compliance Audit Snapshot for Prescription Operations."""
    __tablename__ = "prescriptions_audit_snapshots"
