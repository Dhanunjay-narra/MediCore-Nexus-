"""
MediCore Nexus - Analytics & BI (analytics) SQLAlchemy 2.0 ORM Models
Tables: daily_pharmacy_revenue_aggregations, fast_moving_drug_velocity_metrics, hospital_bed_utilization_snapshots, doctor_throughput_analytics
"""
...
class AnalyticsMasterEntity(Base, TimestampMixin):
    """Primary Master Table for Analytics & BI."""
    __tablename__ = "analytics_master"
...
class AnalyticsTransactionDetail(Base, TimestampMixin):
    """Transactional Line-Item Child Table for Analytics & BI."""
    __tablename__ = "analytics_transactions"

    id: Mapped[str] = mapped_column(String(64), primary_key=True, index=True)
    master_id: Mapped[str] = mapped_column(String(64), ForeignKey("analytics_master.id"), index=True, nullable=False)
...
class AnalyticsAuditSnapshot(Base, TimestampMixin):
    """Historical Compliance Audit Snapshot for Analytics & BI."""
    __tablename__ = "analytics_audit_snapshots"
