"""
MediCore Nexus - Audit & Compliance (audit) SQLAlchemy 2.0 ORM Models
Tables: hipaa_audit_trail_events, electronic_record_access_logs, sensitive_data_export_records, security_policy_override_incidents
"""
...
class AuditMasterEntity(Base, TimestampMixin):
    """Primary Master Table for Audit & Compliance."""
...
class AuditTransactionDetail(Base, TimestampMixin):
    """Transactional Line-Item Child Table for Audit & Compliance."""
    __tablename__ = "audit_transactions"

    id: Mapped[str] = mapped_column(String(64), primary_key=True, index=True)
    master_id: Mapped[str] = mapped_column(String(64), ForeignKey("audit_master.id"), index=True, nullable=False)
...
class AuditAuditSnapshot(Base, TimestampMixin):
    """Historical Compliance Audit Snapshot for Audit & Compliance."""
    __tablename__ = "audit_audit_snapshots"
