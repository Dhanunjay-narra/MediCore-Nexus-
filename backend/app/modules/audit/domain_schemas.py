"""
MediCore Nexus - Audit & HIPAA Compliance (audit) Pydantic v2 Domain Schemas
...
class AuditRecordBaseSchema(BaseModel):
    """Core attributes for AuditRecord."""
...
class AuditRecordCreateSchema(AuditRecordBaseSchema):
    """Payload required to create a new AuditRecord record."""
    is_urgent: Optional[bool] = False
    priority_level: Optional[str] = "Normal"


class AuditRecordUpdateSchema(BaseModel):
    """Payload permitted for partial update of AuditRecord."""
...
class AuditRecordFilterSchema(BaseModel):
    """Query parameters for filtering AuditRecord collections."""
    status: Optional[str] = None
...
class AuditRecordDetailResponseSchema(AuditRecordBaseSchema):
    """Detailed response schema for AuditRecord with audit timestamps."""
    id: str
    created_at: str
    updated_at: str
...
class AuditRecordListResponseSchema(BaseModel):
    """Paginated collection response for AuditRecord."""
    items: List[AuditRecordDetailResponseSchema]
    total: int
    skip: int
    limit: int
    has_more: bool
...
class AuditRecordBulkActionRequest(BaseModel):
    """Batch operation request for AuditRecord entities."""
    action: str = Field(..., description="Action to perform: ACTIVATE, ARCHIVE, DELETE, EXPORT")
    entity_ids: List[str] = Field(..., min_length=1)
    reason: Optional[str] = None


class AuditRecordBulkActionResponse(BaseModel):
