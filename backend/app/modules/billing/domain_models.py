"""
MediCore Nexus - Billing & Revenue (billing) SQLAlchemy 2.0 ORM Models
Tables: hospital_patient_invoices, invoice_itemized_charge_entries, patient_payment_captures, insurance_copay_records, financial_ledger_reconciliations
"""
...
class BillingMasterEntity(Base, TimestampMixin):
    """Primary Master Table for Billing & Revenue."""
...
class BillingTransactionDetail(Base, TimestampMixin):
    """Transactional Line-Item Child Table for Billing & Revenue."""
    __tablename__ = "billing_transactions"

    id: Mapped[str] = mapped_column(String(64), primary_key=True, index=True)
    master_id: Mapped[str] = mapped_column(String(64), ForeignKey("billing_master.id"), index=True, nullable=False)
...
class BillingAuditSnapshot(Base, TimestampMixin):
    """Historical Compliance Audit Snapshot for Billing & Revenue."""
    __tablename__ = "billing_audit_snapshots"
