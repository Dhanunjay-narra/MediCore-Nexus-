"""
MediCore Nexus - Billing & Revenue Ledger (billing) Pydantic v2 Domain Schemas
...
class InvoiceCreateSchema(InvoiceBaseSchema):
    """Payload required to create a new Invoice record."""
    is_urgent: Optional[bool] = False
    priority_level: Optional[str] = "Normal"


class InvoiceUpdateSchema(BaseModel):
    """Payload permitted for partial update of Invoice."""
...
class InvoiceFilterSchema(BaseModel):
    """Query parameters for filtering Invoice collections."""
    status: Optional[str] = None
...
class InvoiceDetailResponseSchema(InvoiceBaseSchema):
    """Detailed response schema for Invoice with audit timestamps."""
    id: str
    created_at: str
    updated_at: str
...
class InvoiceListResponseSchema(BaseModel):
    """Paginated collection response for Invoice."""
    items: List[InvoiceDetailResponseSchema]
    total: int
    skip: int
    limit: int
    has_more: bool
...
class InvoiceBulkActionRequest(BaseModel):
    """Batch operation request for Invoice entities."""
