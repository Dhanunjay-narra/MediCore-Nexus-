"""
MediCore Nexus - Patient Care & MPI (patients) SQLAlchemy 2.0 ORM Models
Tables: patient_entities, patient_demographics, patient_allergies, chronic_conditions, emergency_contacts, guardian_profiles, consent_records
"""
...
class PatientsMasterEntity(Base, TimestampMixin):
    """Primary Master Table for Patient Care & MPI."""
...
class PatientsTransactionDetail(Base, TimestampMixin):
    """Transactional Line-Item Child Table for Patient Care & MPI."""
    __tablename__ = "patients_transactions"

    id: Mapped[str] = mapped_column(String(64), primary_key=True, index=True)
    master_id: Mapped[str] = mapped_column(String(64), ForeignKey("patients_master.id"), index=True, nullable=False)
...
class PatientsAuditSnapshot(Base, TimestampMixin):
    """Historical Compliance Audit Snapshot for Patient Care & MPI."""
    __tablename__ = "patients_audit_snapshots"
