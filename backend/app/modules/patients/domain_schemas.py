"""
MediCore Nexus - Master Patient Index (MPI) (patients) Pydantic v2 Domain Schemas
...
class PatientCreateSchema(PatientBaseSchema):
    """Payload required to create a new Patient record."""
    is_urgent: Optional[bool] = False
    priority_level: Optional[str] = "Normal"


class PatientUpdateSchema(BaseModel):
    """Payload permitted for partial update of Patient."""
...
class PatientFilterSchema(BaseModel):
    """Query parameters for filtering Patient collections."""
    status: Optional[str] = None
...
class PatientDetailResponseSchema(PatientBaseSchema):
    """Detailed response schema for Patient with audit timestamps."""
    id: str
    created_at: str
    updated_at: str
...
class PatientListResponseSchema(BaseModel):
    """Paginated collection response for Patient."""
    items: List[PatientDetailResponseSchema]
    total: int
    skip: int
    limit: int
    has_more: bool
