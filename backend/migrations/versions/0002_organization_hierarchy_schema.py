"""
Alembic Database Migration: Hospital & Organization Hierarchy (organization)
Revision ID: 0002_organization_hierarchy_schema
Revises: '0001_identity_access_baseline'
...
revision = "0002_organization_hierarchy_schema"
down_revision = '0001_identity_access_baseline'
branch_labels = None
depends_on = None


def upgrade() -> None:
    """Execute schema upgrades for Hospital & Organization Hierarchy."""
    op.create_table(
        "organization_master",
    ...
    op.create_index("ix_organization_entity_code", "organization_master", ["entity_code"], unique=True)
    op.create_index("ix_organization_status", "organization_master", ["status"], unique=False)
    op.create_index("ix_organization_hospital", "organization_master", ["hospital_id"], unique=False)

    op.create_table(
        "organization_transactions",
        sa.Column("id", sa.String(length=64), primary_key=True, nullable=False),
        sa.Column("master_id", sa.String(length=64), sa.ForeignKey("organization_master.id"), nullable=False),
    ...
    op.create_index("ix_organization_txn_code", "organization_transactions", ["transaction_code"], unique=True)
