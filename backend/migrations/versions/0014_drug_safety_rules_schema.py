"""
Alembic Database Migration: Clinical Pharmacy & Drug Safety (drug_safety)
Revision ID: 0014_drug_safety_rules_schema
Revises: '0013_sales_pos_transaction_schema'
...
revision = "0014_drug_safety_rules_schema"
down_revision = '0013_sales_pos_transaction_schema'
branch_labels = None
depends_on = None


def upgrade() -> None:
    """Execute schema upgrades for Clinical Pharmacy & Drug Safety."""
    op.create_table(
        "drug_safety_master",
    ...
    op.create_index("ix_drug_safety_entity_code", "drug_safety_master", ["entity_code"], unique=True)
    op.create_index("ix_drug_safety_status", "drug_safety_master", ["status"], unique=False)
    op.create_index("ix_drug_safety_hospital", "drug_safety_master", ["hospital_id"], unique=False)

    op.create_table(
        "drug_safety_transactions",
        sa.Column("id", sa.String(length=64), primary_key=True, nullable=False),
        sa.Column("master_id", sa.String(length=64), sa.ForeignKey("drug_safety_master.id"), nullable=False),
    ...
    op.create_index("ix_drug_safety_txn_code", "drug_safety_transactions", ["transaction_code"], unique=True)


def downgrade() -> None:
    """Revert schema changes."""
    op.drop_table("drug_safety_transactions")
    op.drop_table("drug_safety_master")
