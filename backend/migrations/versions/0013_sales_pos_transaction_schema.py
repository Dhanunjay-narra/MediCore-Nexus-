"""
Alembic Database Migration: Point-of-Sale Engine (sales)
Revision ID: 0013_sales_pos_transaction_schema
Revises: '0012_procurement_po_schema'
...
revision = "0013_sales_pos_transaction_schema"
down_revision = '0012_procurement_po_schema'
branch_labels = None
depends_on = None


def upgrade() -> None:
    """Execute schema upgrades for Point-of-Sale Engine."""
    op.create_table(
        "sales_master",
    ...
    op.create_index("ix_sales_entity_code", "sales_master", ["entity_code"], unique=True)
    op.create_index("ix_sales_status", "sales_master", ["status"], unique=False)
    op.create_index("ix_sales_hospital", "sales_master", ["hospital_id"], unique=False)

    op.create_table(
        "sales_transactions",
        sa.Column("id", sa.String(length=64), primary_key=True, nullable=False),
        sa.Column("master_id", sa.String(length=64), sa.ForeignKey("sales_master.id"), nullable=False),
    ...
    op.create_index("ix_sales_txn_code", "sales_transactions", ["transaction_code"], unique=True)
