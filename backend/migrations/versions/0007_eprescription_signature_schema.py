"""
Alembic Database Migration: Prescription & E-Prescribing (prescriptions)
Revision ID: 0007_eprescription_signature_schema
Revises: '0006_emr_clinical_encounters_schema'
...
revision = "0007_eprescription_signature_schema"
down_revision = '0006_emr_clinical_encounters_schema'
branch_labels = None
depends_on = None


def upgrade() -> None:
    """Execute schema upgrades for Prescription & E-Prescribing."""
    op.create_table(
        "prescriptions_master",
    ...
    op.create_index("ix_prescriptions_entity_code", "prescriptions_master", ["entity_code"], unique=True)
    op.create_index("ix_prescriptions_status", "prescriptions_master", ["status"], unique=False)
    op.create_index("ix_prescriptions_hospital", "prescriptions_master", ["hospital_id"], unique=False)

    op.create_table(
        "prescriptions_transactions",
        sa.Column("id", sa.String(length=64), primary_key=True, nullable=False),
        sa.Column("master_id", sa.String(length=64), sa.ForeignKey("prescriptions_master.id"), nullable=False),
    ...
    op.create_index("ix_prescriptions_txn_code", "prescriptions_transactions", ["transaction_code"], unique=True)


def downgrade() -> None:
    """Revert schema changes."""
    op.drop_table("prescriptions_transactions")
    op.drop_table("prescriptions_master")
