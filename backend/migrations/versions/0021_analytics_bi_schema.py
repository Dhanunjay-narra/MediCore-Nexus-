"""
Alembic Database Migration: Healthcare & Business Intelligence (analytics)
Revision ID: 0021_analytics_bi_schema
Revises: '0020_notifications_dispatch_schema'
...
revision = "0021_analytics_bi_schema"
down_revision = '0020_notifications_dispatch_schema'
branch_labels = None
depends_on = None


def upgrade() -> None:
    """Execute schema upgrades for Healthcare & Business Intelligence."""
    op.create_table(
        "analytics_master",
    ...
    op.create_index("ix_analytics_entity_code", "analytics_master", ["entity_code"], unique=True)
    op.create_index("ix_analytics_status", "analytics_master", ["status"], unique=False)
    op.create_index("ix_analytics_hospital", "analytics_master", ["hospital_id"], unique=False)

    op.create_table(
        "analytics_transactions",
        sa.Column("id", sa.String(length=64), primary_key=True, nullable=False),
        sa.Column("master_id", sa.String(length=64), sa.ForeignKey("analytics_master.id"), nullable=False),
    ...
    op.create_index("ix_analytics_txn_code", "analytics_transactions", ["transaction_code"], unique=True)


def downgrade() -> None:
    """Revert schema changes."""
    op.drop_table("analytics_transactions")
    op.drop_table("analytics_master")
