"""
Alembic Database Migration: Electronic Medical Records & Encounters (emr)
Revision ID: 0006_emr_clinical_encounters_schema
Revises: '0005_appointment_queue_schema'
...
revision = "0006_emr_clinical_encounters_schema"
down_revision = '0005_appointment_queue_schema'
branch_labels = None
depends_on = None


def upgrade() -> None:
    """Execute schema upgrades for Electronic Medical Records & Encounters."""
    op.create_table(
        "emr_master",
    ...
    op.create_index("ix_emr_entity_code", "emr_master", ["entity_code"], unique=True)
    op.create_index("ix_emr_status", "emr_master", ["status"], unique=False)
    op.create_index("ix_emr_hospital", "emr_master", ["hospital_id"], unique=False)

    op.create_table(
        "emr_transactions",
        sa.Column("id", sa.String(length=64), primary_key=True, nullable=False),
        sa.Column("master_id", sa.String(length=64), sa.ForeignKey("emr_master.id"), nullable=False),
    ...
    op.create_index("ix_emr_txn_code", "emr_transactions", ["transaction_code"], unique=True)


def downgrade() -> None:
    """Revert schema changes."""
    op.drop_table("emr_transactions")
    op.drop_table("emr_master")
