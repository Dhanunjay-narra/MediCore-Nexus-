"""
Alembic Database Migration: Pharmacy Inventory & Batch Control (inventory)
Revision ID: 0010_inventory_batch_control_schema
Revises: '0009_pharmacy_dispensing_schema'
...
revision = "0010_inventory_batch_control_schema"
down_revision = '0009_pharmacy_dispensing_schema'
branch_labels = None
depends_on = None


def upgrade() -> None:
    """Execute schema upgrades for Pharmacy Inventory & Batch Control."""
    op.create_table(
        "inventory_master",
    ...
    op.create_index("ix_inventory_entity_code", "inventory_master", ["entity_code"], unique=True)
    op.create_index("ix_inventory_status", "inventory_master", ["status"], unique=False)
    op.create_index("ix_inventory_hospital", "inventory_master", ["hospital_id"], unique=False)

    op.create_table(
        "inventory_transactions",
        sa.Column("id", sa.String(length=64), primary_key=True, nullable=False),
        sa.Column("master_id", sa.String(length=64), sa.ForeignKey("inventory_master.id"), nullable=False),
    ...
    op.create_index("ix_inventory_txn_code", "inventory_transactions", ["transaction_code"], unique=True)
