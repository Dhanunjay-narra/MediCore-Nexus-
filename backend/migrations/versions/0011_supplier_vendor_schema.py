"""
Alembic Database Migration: Supplier & Vendor Contracts (suppliers)
Revision ID: 0011_supplier_vendor_schema
Revises: '0010_inventory_batch_control_schema'
...
revision = "0011_supplier_vendor_schema"
down_revision = '0010_inventory_batch_control_schema'
branch_labels = None
depends_on = None


def upgrade() -> None:
    """Execute schema upgrades for Supplier & Vendor Contracts."""
    op.create_table(
        "suppliers_master",
    ...
    op.create_index("ix_suppliers_entity_code", "suppliers_master", ["entity_code"], unique=True)
    op.create_index("ix_suppliers_status", "suppliers_master", ["status"], unique=False)
    op.create_index("ix_suppliers_hospital", "suppliers_master", ["hospital_id"], unique=False)

    op.create_table(
        "suppliers_transactions",
        sa.Column("id", sa.String(length=64), primary_key=True, nullable=False),
        sa.Column("master_id", sa.String(length=64), sa.ForeignKey("suppliers_master.id"), nullable=False),
    ...
    op.create_index("ix_suppliers_txn_code", "suppliers_transactions", ["transaction_code"], unique=True)


def downgrade() -> None:
    """Revert schema changes."""
    op.drop_table("suppliers_transactions")
    op.drop_table("suppliers_master")
