"""
Alembic Database Migration: Pharmacy Operations & Dispensing (pharmacy)
Revision ID: 0009_pharmacy_dispensing_schema
Revises: '0008_medicine_catalog_schema'
...
revision = "0009_pharmacy_dispensing_schema"
down_revision = '0008_medicine_catalog_schema'
branch_labels = None
depends_on = None


def upgrade() -> None:
    """Execute schema upgrades for Pharmacy Operations & Dispensing."""
    op.create_table(
        "pharmacy_master",
    ...
    op.create_index("ix_pharmacy_entity_code", "pharmacy_master", ["entity_code"], unique=True)
    op.create_index("ix_pharmacy_status", "pharmacy_master", ["status"], unique=False)
    op.create_index("ix_pharmacy_hospital", "pharmacy_master", ["hospital_id"], unique=False)

    op.create_table(
        "pharmacy_transactions",
        sa.Column("id", sa.String(length=64), primary_key=True, nullable=False),
        sa.Column("master_id", sa.String(length=64), sa.ForeignKey("pharmacy_master.id"), nullable=False),
    ...
    op.create_index("ix_pharmacy_txn_code", "pharmacy_transactions", ["transaction_code"], unique=True)


def downgrade() -> None:
    """Revert schema changes."""
    op.drop_table("pharmacy_transactions")
    op.drop_table("pharmacy_master")
