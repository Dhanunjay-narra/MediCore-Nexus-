"""
Alembic Database Migration: Audit & HIPAA Compliance (audit)
Revision ID: 0023_audit_compliance_schema
Revises: '0022_ai_decision_engine_schema'
...
revision = "0023_audit_compliance_schema"
down_revision = '0022_ai_decision_engine_schema'
branch_labels = None
depends_on = None


def upgrade() -> None:
    """Execute schema upgrades for Audit & HIPAA Compliance."""
    op.create_table(
        "audit_master",
    ...
    op.create_index("ix_audit_entity_code", "audit_master", ["entity_code"], unique=True)
    op.create_index("ix_audit_status", "audit_master", ["status"], unique=False)
    op.create_index("ix_audit_hospital", "audit_master", ["hospital_id"], unique=False)

    op.create_table(
        "audit_transactions",
        sa.Column("id", sa.String(length=64), primary_key=True, nullable=False),
        sa.Column("master_id", sa.String(length=64), sa.ForeignKey("audit_master.id"), nullable=False),
    ...
    op.create_index("ix_audit_txn_code", "audit_transactions", ["transaction_code"], unique=True)
