"""
Alembic Database Migration: AI Clinical Decision Support (ai)
Revision ID: 0022_ai_decision_engine_schema
Revises: '0021_analytics_bi_schema'
...
revision = "0022_ai_decision_engine_schema"
down_revision = '0021_analytics_bi_schema'
branch_labels = None
depends_on = None


def upgrade() -> None:
    """Execute schema upgrades for AI Clinical Decision Support."""
    op.create_table(
        "ai_master",
    ...
    op.create_index("ix_ai_entity_code", "ai_master", ["entity_code"], unique=True)
    op.create_index("ix_ai_status", "ai_master", ["status"], unique=False)
    op.create_index("ix_ai_hospital", "ai_master", ["hospital_id"], unique=False)

    op.create_table(
        "ai_transactions",
        sa.Column("id", sa.String(length=64), primary_key=True, nullable=False),
        sa.Column("master_id", sa.String(length=64), sa.ForeignKey("ai_master.id"), nullable=False),
    ...
    op.create_index("ix_ai_txn_code", "ai_transactions", ["transaction_code"], unique=True)
