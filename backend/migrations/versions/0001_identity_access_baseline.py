"""
Alembic Database Migration: Identity & Access Management (identity)
Revision ID: 0001_identity_access_baseline
Revises: None
...
revision = "0001_identity_access_baseline"
down_revision = None
branch_labels = None
depends_on = None
...
def upgrade() -> None:
    """Execute schema upgrades for Identity & Access Management."""
    op.create_table(
        "identity_master",
    ...
    op.create_index("ix_identity_entity_code", "identity_master", ["entity_code"], unique=True)
    op.create_index("ix_identity_status", "identity_master", ["status"], unique=False)
    op.create_index("ix_identity_hospital", "identity_master", ["hospital_id"], unique=False)

    op.create_table(
        "identity_transactions",
        sa.Column("id", sa.String(length=64), primary_key=True, nullable=False),
        sa.Column("master_id", sa.String(length=64), sa.ForeignKey("identity_master.id"), nullable=False),
    ...
    op.create_index("ix_identity_txn_code", "identity_transactions", ["transaction_code"], unique=True)


def downgrade() -> None:
    """Revert schema changes."""
    op.drop_table("identity_transactions")
    op.drop_table("identity_master")
