"""
Alembic Database Migration: Document Vault & Imaging (documents)
Revision ID: 0024_documents_vault_schema
Revises: '0023_audit_compliance_schema'
...
revision = "0024_documents_vault_schema"
down_revision = '0023_audit_compliance_schema'
branch_labels = None
depends_on = None


def upgrade() -> None:
    """Execute schema upgrades for Document Vault & Imaging."""
    op.create_table(
        "documents_master",
    ...
    op.create_index("ix_documents_entity_code", "documents_master", ["entity_code"], unique=True)
    op.create_index("ix_documents_status", "documents_master", ["status"], unique=False)
    op.create_index("ix_documents_hospital", "documents_master", ["hospital_id"], unique=False)

    op.create_table(
        "documents_transactions",
        sa.Column("id", sa.String(length=64), primary_key=True, nullable=False),
        sa.Column("master_id", sa.String(length=64), sa.ForeignKey("documents_master.id"), nullable=False),
    ...
    op.create_index("ix_documents_txn_code", "documents_transactions", ["transaction_code"], unique=True)


def downgrade() -> None:
    """Revert schema changes."""
    op.drop_table("documents_transactions")
    op.drop_table("documents_master")
