"""
Alembic Database Migration: Insurance & Claims Adjudication (insurance)
Revision ID: 0017_insurance_claims_schema
Revises: '0016_billing_invoices_schema'
...
revision = "0017_insurance_claims_schema"
down_revision = '0016_billing_invoices_schema'
branch_labels = None
depends_on = None


def upgrade() -> None:
    """Execute schema upgrades for Insurance & Claims Adjudication."""
    op.create_table(
        "insurance_master",
    ...
    op.create_index("ix_insurance_entity_code", "insurance_master", ["entity_code"], unique=True)
    op.create_index("ix_insurance_status", "insurance_master", ["status"], unique=False)
    op.create_index("ix_insurance_hospital", "insurance_master", ["hospital_id"], unique=False)

    op.create_table(
        "insurance_transactions",
        sa.Column("id", sa.String(length=64), primary_key=True, nullable=False),
        sa.Column("master_id", sa.String(length=64), sa.ForeignKey("insurance_master.id"), nullable=False),
    ...
    op.create_index("ix_insurance_txn_code", "insurance_transactions", ["transaction_code"], unique=True)
