"""
Alembic Database Migration: Billing & Revenue Ledger (billing)
Revision ID: 0016_billing_invoices_schema
Revises: '0015_laboratory_diagnostic_schema'
...
revision = "0016_billing_invoices_schema"
down_revision = '0015_laboratory_diagnostic_schema'
branch_labels = None
depends_on = None


def upgrade() -> None:
    """Execute schema upgrades for Billing & Revenue Ledger."""
    op.create_table(
        "billing_master",
    ...
    op.create_index("ix_billing_entity_code", "billing_master", ["entity_code"], unique=True)
    op.create_index("ix_billing_status", "billing_master", ["status"], unique=False)
    op.create_index("ix_billing_hospital", "billing_master", ["hospital_id"], unique=False)

    op.create_table(
        "billing_transactions",
        sa.Column("id", sa.String(length=64), primary_key=True, nullable=False),
        sa.Column("master_id", sa.String(length=64), sa.ForeignKey("billing_master.id"), nullable=False),
    ...
    op.create_index("ix_billing_txn_code", "billing_transactions", ["transaction_code"], unique=True)
