"""
Alembic Database Migration: Doctor & Specialist Directory (doctors)
Revision ID: 0004_doctor_specialty_schema
Revises: '0003_patient_registry_schema'
...
revision = "0004_doctor_specialty_schema"
down_revision = '0003_patient_registry_schema'
branch_labels = None
depends_on = None


def upgrade() -> None:
    """Execute schema upgrades for Doctor & Specialist Directory."""
    op.create_table(
        "doctors_master",
    ...
    op.create_index("ix_doctors_entity_code", "doctors_master", ["entity_code"], unique=True)
    op.create_index("ix_doctors_status", "doctors_master", ["status"], unique=False)
    op.create_index("ix_doctors_hospital", "doctors_master", ["hospital_id"], unique=False)

    op.create_table(
        "doctors_transactions",
        sa.Column("id", sa.String(length=64), primary_key=True, nullable=False),
        sa.Column("master_id", sa.String(length=64), sa.ForeignKey("doctors_master.id"), nullable=False),
    ...
    op.create_index("ix_doctors_txn_code", "doctors_transactions", ["transaction_code"], unique=True)


def downgrade() -> None:
    """Revert schema changes."""
    op.drop_table("doctors_transactions")
    op.drop_table("doctors_master")
