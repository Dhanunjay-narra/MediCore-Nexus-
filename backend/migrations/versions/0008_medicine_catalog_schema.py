"""
Alembic Database Migration: Medicine Master Catalog (medicines)
Revision ID: 0008_medicine_catalog_schema
Revises: '0007_eprescription_signature_schema'
...
revision = "0008_medicine_catalog_schema"
down_revision = '0007_eprescription_signature_schema'
branch_labels = None
depends_on = None


def upgrade() -> None:
    """Execute schema upgrades for Medicine Master Catalog."""
    op.create_table(
        "medicines_master",
    ...
    op.create_index("ix_medicines_entity_code", "medicines_master", ["entity_code"], unique=True)
    op.create_index("ix_medicines_status", "medicines_master", ["status"], unique=False)
    op.create_index("ix_medicines_hospital", "medicines_master", ["hospital_id"], unique=False)

    op.create_table(
        "medicines_transactions",
        sa.Column("id", sa.String(length=64), primary_key=True, nullable=False),
        sa.Column("master_id", sa.String(length=64), sa.ForeignKey("medicines_master.id"), nullable=False),
    ...
    op.create_index("ix_medicines_txn_code", "medicines_transactions", ["transaction_code"], unique=True)


def downgrade() -> None:
    """Revert schema changes."""
    op.drop_table("medicines_transactions")
    op.drop_table("medicines_master")
