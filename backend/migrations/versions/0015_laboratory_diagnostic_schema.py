"""
Alembic Database Migration: Laboratory Diagnostics (laboratory)
Revision ID: 0015_laboratory_diagnostic_schema
Revises: '0014_drug_safety_rules_schema'
...
revision = "0015_laboratory_diagnostic_schema"
down_revision = '0014_drug_safety_rules_schema'
branch_labels = None
depends_on = None


def upgrade() -> None:
    """Execute schema upgrades for Laboratory Diagnostics."""
    op.create_table(
        "laboratory_master",
    ...
    op.create_index("ix_laboratory_entity_code", "laboratory_master", ["entity_code"], unique=True)
    op.create_index("ix_laboratory_status", "laboratory_master", ["status"], unique=False)
    op.create_index("ix_laboratory_hospital", "laboratory_master", ["hospital_id"], unique=False)

    op.create_table(
        "laboratory_transactions",
        sa.Column("id", sa.String(length=64), primary_key=True, nullable=False),
        sa.Column("master_id", sa.String(length=64), sa.ForeignKey("laboratory_master.id"), nullable=False),
    ...
    op.create_index("ix_laboratory_txn_code", "laboratory_transactions", ["transaction_code"], unique=True)


def downgrade() -> None:
    """Revert schema changes."""
    op.drop_table("laboratory_transactions")
    op.drop_table("laboratory_master")
