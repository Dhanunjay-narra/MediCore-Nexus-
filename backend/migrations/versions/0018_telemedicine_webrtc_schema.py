"""
Alembic Database Migration: Telemedicine Suite (telemedicine)
Revision ID: 0018_telemedicine_webrtc_schema
Revises: '0017_insurance_claims_schema'
...
revision = "0018_telemedicine_webrtc_schema"
down_revision = '0017_insurance_claims_schema'
branch_labels = None
depends_on = None


def upgrade() -> None:
    """Execute schema upgrades for Telemedicine Suite."""
    op.create_table(
        "telemedicine_master",
    ...
    op.create_index("ix_telemedicine_entity_code", "telemedicine_master", ["entity_code"], unique=True)
    op.create_index("ix_telemedicine_status", "telemedicine_master", ["status"], unique=False)
    op.create_index("ix_telemedicine_hospital", "telemedicine_master", ["hospital_id"], unique=False)

    op.create_table(
        "telemedicine_transactions",
        sa.Column("id", sa.String(length=64), primary_key=True, nullable=False),
        sa.Column("master_id", sa.String(length=64), sa.ForeignKey("telemedicine_master.id"), nullable=False),
    ...
    op.create_index("ix_telemedicine_txn_code", "telemedicine_transactions", ["transaction_code"], unique=True)


def downgrade() -> None:
    """Revert schema changes."""
    op.drop_table("telemedicine_transactions")
    op.drop_table("telemedicine_master")
