"""
Alembic Database Migration: Master Patient Index & Demographics (patients)
Revision ID: 0003_patient_registry_schema
Revises: '0002_organization_hierarchy_schema'
...
revision = "0003_patient_registry_schema"
down_revision = '0002_organization_hierarchy_schema'
branch_labels = None
depends_on = None


def upgrade() -> None:
    """Execute schema upgrades for Master Patient Index & Demographics."""
    op.create_table(
        "patients_master",
    ...
    op.create_index("ix_patients_entity_code", "patients_master", ["entity_code"], unique=True)
    op.create_index("ix_patients_status", "patients_master", ["status"], unique=False)
    op.create_index("ix_patients_hospital", "patients_master", ["hospital_id"], unique=False)

    op.create_table(
        "patients_transactions",
        sa.Column("id", sa.String(length=64), primary_key=True, nullable=False),
        sa.Column("master_id", sa.String(length=64), sa.ForeignKey("patients_master.id"), nullable=False),
    ...
    op.create_index("ix_patients_txn_code", "patients_transactions", ["transaction_code"], unique=True)


def downgrade() -> None:
    """Revert schema changes."""
    op.drop_table("patients_transactions")
    op.drop_table("patients_master")
